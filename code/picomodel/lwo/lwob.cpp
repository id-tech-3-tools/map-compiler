...
static int add_clip( char *s, lwClip **clist, int *nclips ){
	...
	clip = static_cast<lwClip*>(_pico_calloc(1, sizeof( lwClip)));
	if ( !clip ) {
		return 0;
	}
	...
	lwListAdd( reinterpret_cast<void **>(clist), clip );
...
static int add_tvel( float pos[], float vel[], lwEnvelope **elist, int *nenvs ){
	...
	for ( i = 0; i < 3; i++ ) {
		env = static_cast<lwEnvelope*>(_pico_calloc(1, sizeof( lwEnvelope)));
		key0 = reinterpret_cast<lwKey*>(1, sizeof( lwKey));
		key1 = reinterpret_cast<lwKey*>(1, sizeof( lwKey));
		if ( !env || !key0 || !key1 ) {
			return 0;
		}
		...
		env->index = *nenvs + i + 1;
		env->type = 0x0301 + i;
		env->name = static_cast<char*>(_pico_alloc(11));
		if ( env->name ) {
		...
		lwListAdd( reinterpret_cast<void **>(elist), env );
	}
...
static lwTexture *get_texture( char *s ){
	...
	tex = static_cast<lwTexture*>(_pico_calloc(1, sizeof( lwTexture)));
	if ( !tex ) {
		return NULL;
	}
...
lwSurface *lwGetSurface5( picoMemStream_t *fp, int cksize, lwObject *obj ){
	...
	surf = static_cast<lwSurface*>(_pico_calloc(1, sizeof( lwSurface)));
	if ( !surf ) {
		goto Fail;
	}
	...
	while ( 1 ) {
		...
		case ID_BTEX:
			s = static_cast<char*>(getbytes(fp, sz));
			tex = get_texture( s );
			lwListAdd( reinterpret_cast<void **>(&surf->bump.tex), tex );
			break;

		case ID_CTEX:
			s = static_cast<char*>(getbytes(fp, sz));
			tex = get_texture( s );
			lwListAdd( reinterpret_cast<void **>(&surf->color.tex), tex );
			break;

		case ID_DTEX:
			s = static_cast<char*>(getbytes(fp, sz));
			tex = get_texture( s );
			lwListAdd( reinterpret_cast<void **>(&surf->diffuse.tex), tex );
			break;

		case ID_LTEX:
			s = static_cast<char*>(getbytes(fp, sz));
			tex = get_texture( s );
			lwListAdd( reinterpret_cast<void **>(&surf->luminosity.tex), tex );
			break;

		case ID_RTEX:
			s = static_cast<char*>(getbytes(fp, sz));
			tex = get_texture( s );
			lwListAdd( reinterpret_cast<void **>(&surf->reflection.val.tex), tex );
			break;

		case ID_STEX:
			s = static_cast<char*>(getbytes(fp, sz));
			tex = get_texture( s );
			lwListAdd( reinterpret_cast<void **>(&surf->specularity.tex), tex );
			break;

		case ID_TTEX:
			s = static_cast<char*>(getbytes(fp, sz));
			tex = get_texture( s );
			lwListAdd( reinterpret_cast<void **>(&surf->transparency.val.tex), tex );
			break;
		...
		case ID_TREF:
			...
			tex->tmap.ref_object = static_cast<char*>(getbytes(fp, sz));
			break;
		...
		case ID_SHDR:
			shdr = static_cast<lwPlugin*>(_pico_calloc(1, sizeof( lwPlugin)));
			if ( !shdr ) {
				goto Fail;
			}
			shdr->name = static_cast<CHAR*>(getbytes(fp, sz));
			lwListAdd( reinterpret_cast<void**>(&surf->shader), shdr );
			surf->nshaders++;
			break;
...
int lwGetPolygons5( picoMemStream_t *fp, int cksize, lwPolygonList *plist, int ptoffset ){
	...
	set_flen( 0 );
	buf = static_cast<unsigned char*>(getbytes(fp, cksize));
	if ( !buf ) {
		goto Fail;
	}
...
lwObject *lwGetObject5( const char *filename, picoMemStream_t *fp, unsigned int *failID, int *failpos ){
	...
	object = static_cast<lwObject*>(_pico_calloc(1, sizeof( lwObject)));
	if ( !object ) {
		goto Fail;
	}

	layer = static_cast<lwLayer*>(_pico_calloc(1, sizeof( lwLayer)));
	if ( !layer ) {
		goto Fail;
	}
	object->layer = layer;
	...
	while ( 1 ) {
		...
		case ID_SURF:
			...
			lwListAdd( reinterpret_cast<void **>(&object->surf), node );
			object->nsurfs++;
			break;
