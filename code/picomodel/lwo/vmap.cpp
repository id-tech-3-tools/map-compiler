...
lwVMap *lwGetVMap( picoMemStream_t *fp, int cksize, int ptoffset, int poloffset,
	...
	vmap = static_cast<lwVMap*>(_pico_calloc(1, sizeof( lwVMap)));
	if ( !vmap ) {
		_pico_free( buf );
		return NULL;
	}
	...
	vmap->nverts = npts;
	vmap->vindex = static_cast<int*>(_pico_calloc(npts, sizeof( int)));
	if ( !vmap->vindex ) {
		goto Fail;
	}
	if ( perpoly ) {
		vmap->pindex = static_cast<int*>(_pico_calloc(npts, sizeof( int)));
		if ( !vmap->pindex ) {
			goto Fail;
		}
	}
	...
	if ( vmap->dim > 0 ) {
		vmap->val = static_cast<float**>(_pico_calloc(npts, sizeof( float *)));
		if ( !vmap->val ) {
			goto Fail;
		}
		f = static_cast<float*>(_pico_alloc(npts * vmap->dim * sizeof( float)));
		if ( !f ) {
			goto Fail;
		}
		for ( i = 0; i < npts; i++ )
...
int lwGetPointVMaps( lwPointList *point, lwVMap *vmap ){
	...
	for ( i = 0; i < point->count; i++ ) {
		if ( point->pt[ i ].nvmaps ) {
			point->pt[ i ].vm = static_cast<lwVMapPt*>(_pico_calloc(point->pt[ i ].nvmaps, sizeof( lwVMapPt)));
			if ( !point->pt[ i ].vm ) {
				return 0;
			}
			point->pt[ i ].nvmaps = 0;
...
int lwGetPolyVMaps( lwPolygonList *polygon, lwVMap *vmap ){
	...
	for ( i = 0; i < polygon->count; i++ ) {
		for ( j = 0; j < polygon->pol[ i ].nverts; j++ ) {
			...
			if ( pv->nvmaps ) {
				pv->vm = static_cast<lwVMapPt*>(_pico_calloc(pv->nvmaps, sizeof( lwVMapPt)));
				if ( !pv->vm ) {
					return 0;
				}
				pv->nvmaps = 0;
