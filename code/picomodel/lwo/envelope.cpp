...
lwEnvelope *lwGetEnvelope( picoMemStream_t *fp, int cksize ){
	...
	while ( 1 ) {
		...
		case ID_KEY:
			key = static_cast<lwKey*>(_pico_calloc(1, sizeof( lwKey)));
			if ( !key ) {
				goto Fail;
			}
			key->time = getF4( fp );
		...
		case ID_CHAN:
			plug = static_cast<lwPlugin*>(_pico_calloc(1, sizeof( lwPlugin)));
			if ( !plug ) {
				goto Fail;
			}
			...
			lwListAdd( reinterpret_cast<void**>(&env->cfilter), plug );
			env->ncfilters++;
			break;
