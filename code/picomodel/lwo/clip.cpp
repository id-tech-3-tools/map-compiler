...
lwClip *lwGetClip( picoMemStream_t *fp, int cksize ){
	...
	clip = reinterpret_cast<lwClip*>(1, sizeof( lwClip));
	if ( !clip ) {
		goto Fail;
	}
	...
	while ( 1 ) {
		...
		case ID_IFLT:
		case ID_PFLT:
			filt = static_cast<lwPlugin*>(_pico_calloc(1, sizeof( lwPlugin)));
			if ( !filt ) {
				goto Fail;
			}
			...
			if ( id == ID_IFLT ) {
				lwListAdd( reinterpret_cast<void**>(&clip->ifilter), filt );
				clip->nifilters++;
			}
			else {
				lwListAdd( reinterpret_cast<void**>(&clip->pfilter), filt );
				clip->npfilters++;
			}
			break;
