...
int lwGetPoints( picoMemStream_t *fp, int cksize, lwPointList *point ){
	...
	point->count += np;
	if ( !_pico_realloc( reinterpret_cast<void**>(&point->pt), ( point->count - np ) * sizeof( lwPoint ), point->count * sizeof( lwPoint ) ) ) {
		return 0;
	}
...
int lwAllocPolygons( lwPolygonList *plist, int npols, int nverts ){
	...
	plist->count += npols;
	if ( !_pico_realloc( reinterpret_cast<void**>(&plist->pol), ( plist->count - npols ) * sizeof( lwPolygon ), plist->count * sizeof( lwPolygon ) ) ) {
		return 0;
	}
	...
	plist->vcount += nverts;
	if ( !_pico_realloc( reinterpret_cast<void**>(&plist->pol[ 0 ].v), ( plist->vcount - nverts ) * sizeof( lwPolVert ), plist->vcount * sizeof( lwPolVert ) ) ) {
		return 0;
	}
...
int lwGetPolygons( picoMemStream_t *fp, int cksize, lwPolygonList *plist, int ptoffset ){
	...
	set_flen( 0 );
	type = getU4( fp );
	buf = static_cast<unsigned char*>(getbytes(fp, cksize - 4));
	if ( cksize != get_flen() ) {
		goto Fail;
	}
...
int lwGetPointPolygons( lwPointList *point, lwPolygonList *polygon ){
	...
	for ( i = 0; i < point->count; i++ ) {
		...
		point->pt[ i ].pol = static_cast<int*>(_pico_calloc(point->pt[ i ].npols, sizeof( int)));
		if ( !point->pt[ i ].pol ) {
			return 0;
		}
		point->pt[ i ].npols = 0;
...
int lwResolvePolySurfaces( lwPolygonList *polygon, lwTagList *tlist,
	...
	s = static_cast<lwSurface**>(_pico_calloc(tlist->count, sizeof( lwSurface *)));
	if ( !s ) {
		return 0;
	}
	...
	for ( i = 0; i < polygon->count; i++ ) {
		...
		if ( !s[ index ] ) {
			...
			s[ index ]->name = static_cast<char*>(_pico_alloc(strlen(tlist->tag[ index ]) + 1));
			if ( !s[ index ]->name ) {
				return 0;
			}
			strcpy( s[ index ]->name, tlist->tag[ index ] );
			lwListAdd( reinterpret_cast<void**>(surf), s[ index ] );
			*nsurfs = *nsurfs + 1;
		}
...
int lwGetTags( picoMemStream_t *fp, int cksize, lwTagList *tlist ){
	...
	set_flen( 0 );
	buf = static_cast<char*>(getbytes(fp, cksize));
	if ( !buf ) {
		return 0;
	}
	...
	tlist->count += ntags;
	if ( !_pico_realloc( reinterpret_cast<void**>(&tlist->tag), ( tlist->count - ntags ) * sizeof( char * ), tlist->count * sizeof( char * ) ) ) {
		goto Fail;
	}
