...
int WINS_OpenReliableSocket( int port ){
	...
	//
	if ( setsockopt( newsocket, IPPROTO_TCP, TCP_NODELAY, reinterpret_cast<char*>(&_true), sizeof( int ) ) == SOCKET_ERROR ) {
		WinPrint( "WINS_OpenReliableSocket: %s\n", WINS_ErrorMessage( WSAGetLastError() ) );
...
int WINS_Read( int socket, byte *buf, int len, struct sockaddr_s *addr ){
	...
	{
		ret = recv( socket, reinterpret_cast<char*>(buf), len, 0 );
		if ( ret == SOCKET_ERROR ) {
			errno = WSAGetLastError();
...
int WINS_Write( int socket, byte *buf, int len, struct sockaddr_s *addr ){
	...
	if ( addr ) {
		...
		while ( written < len )
		{
			ret = sendto( socket, reinterpret_cast<char*>(&buf[written]), len - written, 0, (struct sockaddr *)addr, sizeof( struct sockaddr_s ) );
			if ( ret == SOCKET_ERROR ) {
				if ( WSAGetLastError() != WSAEWOULDBLOCK ) {
					return qfalse;
				}
	...
	else
	{
		...
		while ( written < len )
		{
			ret = send( socket, reinterpret_cast<char*>(buf), len, 0 );
			if ( ret == SOCKET_ERROR ) {
				if ( WSAGetLastError() != WSAEWOULDBLOCK ) {
					return qfalse;
				}
