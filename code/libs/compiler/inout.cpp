...
void xml_Select( char *msg, int entitynum, int brushnum, qboolean bError ){
	...
	//sprintf_s( buf, 1024, "Entity %i, Brush %i: %s", entitynum, brushnum, msg );
	...
	//sprintf_s( buf, 1024, "%i %i", entitynum, brushnum );
...
void xml_Point( char *msg, vec3_t pt ){
	...
	sprintf_s( buf, 1024, "%s (%g %g %g)", msg, pt[0], pt[1], pt[2] );
	Error( buf );
}
...
void Sys_FPrintf( int flag, const char *format, ... ){
	...
	vsprintf_s( out_buffer, 4096, format, argptr );
	va_end( argptr );
...
void Error( const char *error, ... ){
	...
	vsprintf_s( tmp, 4096, error, argptr );
	va_end( argptr );

	sprintf_s( out_buffer, 4096, "************ ERROR ************\n%s\n", tmp );
