...
set OutputName=%baseName%.ase
set OutputPath=%OutputDirectory%%OutputName%
if not exist "%OutputDirectory%" mkdir "%OutputDirectory%"

:: Additional switches
set ExtraSwitches=-shaderasbitmap -meta -patchmeta

:: Compose compiler argument string, excluding mapname
set CompilerArguments=-convert -game %GameName% -fs_basepath "%GameDirectory%" -v -format ase %ExtraSwitches%
...
:printHelp
    echo Basic usage:
    echo    bsp-to-ase.bat mapname [output relative path]
    echo    bsp-to-ase.bat castle.bsp
    echo    bsp-to-ase.bat castle.bsp models/castle
    goto atExit
